...
const PatientLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const doctorDocRef = doc(db, 'patients', user.uid); 
  ...
  return (
    <div className="flex flex-col justify-center items-center h-screen">
      <form onSubmit={handleLogin} className="flex flex-col bg-white p-6 rounded shadow-md w-80">
        <h2 className="text-lg font-bold mb-4">Patient Login</h2>
        ...
        <p>If you don't have an account <Link to="/patientSignup" className="text-blue-500 underline">Sign Up</Link></p>
      </form>
    </div>
  );
};
